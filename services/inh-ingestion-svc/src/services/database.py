    ...
    async def store_processed_document(
        ...
        with self.get_session() as session:
            try:
                ...
                # Chunks created/edited via the public-api chunk CRUD endpoints
                # (#133) live in this same table with no workflow/run marker,
                # so the wholesale delete below would silently discard them.
                # There is no cross-service lock preventing that here; at
                # minimum make the loss loud instead of silent so it can be
