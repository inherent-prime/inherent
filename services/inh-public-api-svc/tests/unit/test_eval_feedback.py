...
EVENT = {
    "event_id": "ev_1", "workspace_id": "ws-1", "query_text": "refund policy",
    "search_mode": "hybrid",
    "result_doc_ids": ["d1", "d2"], "result_chunk_ids": ["c1", "c2"],
}
...
async def test_unknown_event_raises():
    ...
    with pytest.raises(EventNotFoundError):
        await submit_feedback(db, workspace_ids=["ws-1"],
                              req=FeedbackRequest(event_id="ev_x", verdict="answered"))
...
async def test_partial_without_chunks_does_not_promote():
    db = _db()
    resp = await submit_feedback(db, workspace_ids=["ws-1"],
                                 req=FeedbackRequest(event_id="ev_1", verdict="partial"))
...
async def test_not_relevant_records_but_never_promotes():
    db = _db()
    resp = await submit_feedback(db, workspace_ids=["ws-1"],
                                 req=FeedbackRequest(event_id="ev_1", verdict="not_relevant"))
