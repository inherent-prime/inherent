...
def _response(n=1):
    results = [
        SearchResult(chunk_id=f"c{i}", document_id=f"d{i}", document_name=f"doc{i}",
                     content="x", score=0.9 - i * 0.1)
        for i in range(n)
    ]
    return SearchResponse(results=results, query="q", total_results=n,
                          processing_time_ms=12.5, search_mode="hybrid")
